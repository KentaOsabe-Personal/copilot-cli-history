...
**アーキテクチャ統合**:
- 採用パターン: frontend presentation extension。既存 API DTO をそのまま使い、表示用 helper、component-local state、hook-local integration を拡張する。
- 依存方向: `sessionApi.types` → `presentation helpers` → `components` → `pages`。hooks は `sessionApi` と pages の間に置き、components は hook や page を import しない。
- 維持する既存パターン: relative import、feature 近傍 test、Tailwind utility styling、lightweight SPA、read-only API 利用。
- 新規コンポーネント方針: 新規境界は追加せず、既存 file に表示契約を足す。必要な test file だけを近傍に追加する。
- ステアリング適合: local-first / read-only / current-legacy 共通 contract / lightweight SPA の方針を維持する。
...
| 項目 | 詳細 |
|------|------|
| 目的 | 入力 timestamp を成功時だけ JST として識別できる固定表示へ変換する |
...
```
- 事前条件: `value` は backend DTO の nullable timestamp または欠損値である。
- 事後条件: 成功変換時だけ JST 表示を返す。
- 不変条件: 欠落値と invalid value は JST 成功表示として扱わない。

#### `conversationContent`

| 項目 | 詳細 |
|------|------|
| 目的 | 発話本文、code block、tool hint を表示用 block へ変換し、tool arguments の初期折りたたみ方針を決める |
| 要件 | 4.1, 4.2, 4.3, 6.4 |
...
```
- 事前条件: entry は API DTO の `SessionConversationEntry` である。
- 事後条件: tool call の存在は arguments が collapsed でも block として残る。
- 不変条件: backend DTO の内容を変更せず、表示用 metadata だけを追加する。
...
| 項目 | 詳細 |
|------|------|
| 目的 | 会話 transcript を role 別に識別可能な発話 card として描画し、発話単位の表示状態を制御する |
| 要件 | 2.1, 2.2, 2.3, 2.4, 2.5, 3.1, 3.2, 3.3, 3.4, 3.5, 6.3 |
...
**依存関係**
- 入力元: `SessionDetailPage` — conversation DTO を渡す (P0)
- 出力先: `formatConversationEntryContent` — visual blocks を導出する (P0)
- 出力先: `formatTimestamp` — 発話日時を JST 表示する (P0)
- 出力先: `TimelineContent` — visible entry の content を描画する (P0)
- 出力先: `IssueList` — visible entry の issue を描画する (P0)

**契約**: Service [ ] / API [ ] / Event [ ] / Batch [ ] / State [x]

##### 状態管理
- 状態モデル: `Readonly<Record<number, boolean>>` または同等の sequence keyed map。`true` は visible、`false` は collapsed。
- 永続性と整合性: component lifetime のみ。conversation prop が変わった場合、存在しない sequence の状態は使わない。
- 競合時の方針: user click のみで変更し、remote fetch state と同期しない。

#### `TimelineContent`

| 項目 | 詳細 |
|------|------|
| 目的 | text / code / detail / tool hint block を描画し、tool arguments preview の disclosure を管理する |
| 要件 | 2.5, 3.2, 3.3, 4.1, 4.2, 4.3, 4.4, 4.5, 6.4 |
...
**依存関係**
- 入力元: `ConversationTranscript`, `TimelineEntry`, `ActivityTimeline` — event-like payload を渡す (P0)
- 出力先: `formatTimelineContent` — visual blocks を導出する (P0)

**契約**: Service [ ] / API [ ] / Event [ ] / Batch [ ] / State [x]

##### 状態管理
- 状態モデル: block key ごとの `expanded` boolean。初期値は `!argumentsDefaultCollapsed`。
- 永続性と整合性: component lifetime のみ。発話が非表示になると mounted block state は破棄されてもよい。
- 競合時の方針: local click のみで変更し、backend state と同期しない。

#### `useSessionIndex`

| 項目 | 詳細 |
|------|------|
| 目的 | 一覧取得 state を管理し、同一閲覧 flow の直近 success / empty を即時再利用する |
...
**依存関係**
- 入力元: `SessionIndexPage` — hook result を表示に使う (P0)
- 出力先: `SessionApiClient.fetchSessionIndex` — read-only list fetch (P0)
- 外部: `AbortController` — unmount 時の in-flight request abort (P0)
...
```
- 事前条件: `client` は `SessionApiClient` contract を満たす。
- 事後条件: success / empty は即時再利用可能な snapshot として保存される。
- 不変条件: hook は backend order を変更せず、error を cache しない。

##### 状態管理
- 状態モデル: module-scope `SessionIndexCacheSnapshot | null` と hook-local settled state。
- 永続性と整合性: browser process 内の module lifetime のみ。reload、tab 間共有、storage 永続化はしない。
- 競合時の方針: unmount 時 abort と existing active flag を維持し、古い response が unmounted hook を更新しない。
